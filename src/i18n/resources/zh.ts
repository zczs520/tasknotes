...
export const zh: TranslationTree = {
	onboarding: {
		restoreExamples: "创建或恢复示例任务",
		restoreExamplesHint:
			"补齐工作、学习、创作、健身、家务、娱乐六个双语示例及功能说明，保留已有文件的修改。",
		examplesRestored: "示例任务已就绪，请打开 TASKquence/Tasks 查看。",

		statusSaved: "编辑成功，看板已刷新。",
		statusDeleted: "删除成功，看板已刷新。",
		fixedIdentity: "任务识别、看板和时间统计统一使用同一项任务属性。",
		fixedIdentityHint:
			"任务必须有 taskType 属性，值为 task。此规则固定不可修改；tags 标签用于分类和看板泳道。",
		enableView: "启用此视图及其快捷入口",
		editStatus: "编辑状态",
		deleteStatus: "删除状态",
		statusColor: "颜色",
		statusLabelRequired: "请填写状态名称。",
		keepOneStatus: "至少保留一个状态。",
		deleteStatusHint:
			"此操作会从所有看板和插件设置中移除该状态。任务笔记及原状态值会保留；可将任务拖到其他列重新分配状态。",
		viewDisabled: "请在“设置 → TaskNotes Time → 常规 → 视图与 Base 文件”中启用此视图。",
	},
	common: {
	...
	views: {
		...
		notes: {
			...
			empty: {
				noNotesFound: "未找到笔记",
				helpText:
					"未找到所选日期的笔记。尝试在迷你日历视图中选择不同的日期或创建一些笔记。",
			},
			loading: "加载笔记中...",
		...
		kanban: {
			...
			errors: {
				...
				noGroupBy:
					"看板视图需要配置「分组依据」属性。点击「排序」按钮，然后在「分组依据」下选择一个属性。",
				formulaGroupingReadOnly:
					"无法在基于公式的列之间移动任务。公式值是计算得出的，不能直接修改。",
				formulaSwimlaneReadOnly:
					"无法在基于公式的泳道之间移动任务。公式值是计算得出的，不能直接修改。",
			},
			columnTitle: "无标题",
		...
		releaseNotes: {
			title: "TaskNotes Time {version} 的新功能",
			header: "TaskNotes Time {version} 的新功能",
		...
		basesViewSettings: {
			viewNames: {
					taskList: "TaskNotes Time 任务列表",
					kanban: "TaskNotes Time 看板",
					calendar: "TaskNotes Time 日历",
					miniCalendar: "TaskNotes Time 迷你日历",
					timeStatistics: "TaskNotes Time 时间统计",
			},
			common: {
				enableSearch: "启用搜索框",
	...
	settings: {
		...
		modalFields: {
			...
			splitLayout: {
				name: "宽屏时使用左右分栏",
				description:
					"启用后，屏幕宽度达到 900px 时，详情编辑器显示在右栏；关闭后，弹窗使用上下堆叠布局。",
			},
			tabMovesFocus: {
				name: "在详情编辑器中用 Tab 切换字段",
				description:
					"启用后，Tab 从详情编辑器移到下一个弹窗字段，Shift+Tab 移到上一个字段；关闭后，Tab 和 Shift+Tab 保持 Markdown 编辑器的缩进操作。",
			},
			sync: {
				name: "同步自定义字段",
		...
		features: {
			...
			instantConvert: {
				...
				folder: {
					name: "内联创建任务的文件夹",
					description:
						"通过内联命令或复选框转换创建的任务将存放在此文件夹。留空则使用默认任务文件夹。使用 {{currentNotePath}} 表示当前笔记的文件夹，或使用 {{currentNoteTitle}} 表示以当前笔记命名的子文件夹。",
				},
			},
			nlp: {
				header: "自然语言处理",
			...
			uiLanguage: {
				header: "界面语言",
				description: "更改 TaskNotes Time 菜单、通知和视图的语言。",
				dropdown: {
					name: "界面语言",
					description: "选择 TaskNotes Time 界面文本使用的语言",
				},
			},
			pomodoroSound: {
				enabledName: "启用声音",
			...
			recurring: {
				...
				resetCheckboxesDesc:
					"当重复任务完成并重新安排时，重置任务正文中的所有markdown复选框",
			},
			timeblocking: {
				header: "时间块",
				description:
					"配置时间块功能，在日记中进行轻量级调度。在日历视图上拖动以创建事件 - 从上下文菜单中选择'时间块'。",
				enableName: "启用时间块",
				enableDesc:
					"启用时间块功能，在日记中进行轻量级调度。启用后，'时间块'选项会出现在日历拖动上下文菜单中。",
		...
		defaults: {
			...
			bodyTemplate: {
				...
				useOccurrenceBodyTemplate: {
					name: "使用实例笔记模板",
					description:
						"当重复任务未设置 occurrence_template 时，为已物化的实例笔记使用单独的备用模板",
				},
				occurrenceBodyTemplateFile: {
					name: "实例笔记模板文件",
					description:
						"已物化实例笔记的模板文件路径。重复任务的 occurrence_template 字段优先于此备用模板。",
		...
		general: {
			taskStorage: {
				...
				defaultFolder: {
					name: "默认任务文件夹",
					description:
						"新任务的默认位置。支持 {{currentNotePath}}、{{currentNoteTitle}} 和 {{projectFilePath}} 等文件夹模板变量，也支持 YYYY/MM/DD 等 Daily Notes 风格日期标记。",
				},
				moveArchived: {
					name: "将归档任务移动到文件夹",
				...
				archiveFolder: {
					name: "归档文件夹",
					description:
						"归档时将任务移动到的文件夹。支持模板变量，如 {{year}}、{{month}}、{{priority}} 等。",
				},
			},
			taskIdentification: {
				header: "任务识别",
				...
				taskTag: {
					name: "任务标签",
					description:
						"识别笔记为任务的标签（不含 #）。更改此设置后，现有 .base 视图筛选器会保留旧标签；请更新默认 Base 文件或手动编辑这些筛选器。",
				},
				hideIdentifyingTags: {
					name: "在任务卡片中隐藏识别标签",
					description:
						"启用后，与任务识别标签匹配的标签（包括层次匹配，如 'task/project'）将在任务卡片显示中隐藏",
				},
			...
			frontmatter: {
				...
				useMarkdownLinks: {
					...
					description:
						"在 frontmatter 属性中生成 markdown 链接 ([文本](路径)) 而不是 wikilinks ([[链接]])。\\n\\n⚠️ 需要 'obsidian-frontmatter-markdown-links' 插件才能正常工作。",
				},
			},
			taskInteraction: {
				header: "任务交互",
			...
			releaseNotes: {
				...
				checkForUpdates: {
					name: "启动时检查新版本",
					description:
						"TaskNotes 启动时检查一次 GitHub，并在有较新的兼容版本可用时显示通知",
				},
				viewButton: {
					name: "查看版本说明",
		...
		taskProperties: {
			...
			titleCard: {
				...
				legacySyntaxWarning:
					"像 {title} 这样的单花括号语法已弃用。请使用双花括号语法 {{title}} 以与正文模板保持一致。",
			},
			tagsCard: {
		...
		appearance: {
			...
			taskFilenames: {
				...
				customTemplate: {
					name: "自定义文件名模板",
					description:
						"自定义文件名模板。可用变量：{{title}}, {{titleLower}}, {{titleUpper}}, {{titleSnake}}, {{titleKebab}}, {{titleCamel}}, {{titlePascal}}, {{date}}, {{shortDate}}, {{time}}, {{time12}}, {{time24}}, {{timestamp}}, {{dateTime}}, {{year}}, {{month}}, {{monthName}}, {{monthNameShort}}, {{day}}, {{dayName}}, {{dayNameShort}}, {{hour}}, {{hour12}}, {{minute}}, {{second}}, {{milliseconds}}, {{ms}}, {{ampm}}, {{week}}, {{quarter}}, {{unix}}, {{unixMs}}, {{timezone}}, {{timezoneShort}}, {{utcOffset}}, {{utcOffsetShort}}, {{utcZ}}, {{zettel}}, {{uuid}}, {{nano}}, {{priority}}, {{priorityShort}}, {{status}}, {{statusShort}}, {{dueDate}}, {{scheduledDate}}",
			...
			calendarView: {
				...
				calendarLocale: {
					...
					placeholder: "自动检测",
					invalidLocale:
						"无效的区域设置。请输入有效的语言标签（例如：'zh'、'en'、'fr-FR'）。",
				},
			},
			defaultEventVisibility: {
				header: "默认事件可见性",
				description:
					"配置打开日历时默认可见的事件类型。用户仍可在日历视图中切换这些开/关。",
				showScheduledTasks: {
					name: "显示安排的任务",
			...
			timeSettings: {
				...
				endTime: {
					name: "结束时间",
					description:
						"时间线视图中显示的最晚时间（HH:MM 格式）。使用大于 24:00 的值可显示次日凌晨时间，例如 26:00 表示凌晨 2 点。",
					placeholder: "26:00",
				},
				initialScrollTime: {
					name: "初始滚动时间",
		...
		integrations: {
			...
			basesIntegration: {
				...
				viewCommands: {
					header: "视图与 Base 文件",
					description:
						"TaskNotes 使用 Obsidian Bases 文件（.base）来呈现视图。这些文件在启动时自动生成（如果不存在），并使用您当前的设置进行配置（任务识别、字段映射、状态等）。",
					descriptionRegen:
						"更改设置时，Base 文件不会自动更新。要应用新设置，请使用下方的“更新文件”，删除现有 .base 文件并重启 Obsidian，或手动编辑它们。",
				...
				autoCreateDefaultFiles: {
					name: "自动创建默认文件",
					description:
						"启动时自动创建缺失的默认 Base 视图文件。禁用以防止已删除的示例文件被重新创建。",
				},
				createDefaultFiles: {
					name: "创建默认文件",
					description:
						"在 TaskNotes/Views/ 目录中创建默认的 .base 文件。现有文件不会被覆盖。",
					buttonText: "创建文件",
				},
				exportV3Views: {
					name: "将V3保存的视图导出到Bases",
					description:
						"将您所有来自 TaskNotes v3 的保存视图转换为包含多个视图的单个 .base 文件。这有助于将您的 v3 过滤器配置迁移到新的 Bases 系统。",
					buttonText: "导出V3视图",
					noViews: "没有保存的视图可导出",
					fileExists: "文件已存在",
				...
				updateDefaultFiles: {
					...
					confirmMessage:
						"这会用新生成的模板覆盖已配置的默认 .base 文件。这些文件中的任何手动编辑都将被替换。",
					confirmText: "更新文件",
				},
			},
			calendarSubscriptions: {
				...
				useICSEndAsDue: {
					...
					description:
						"启用后，从日历事件创建的任务将把截止日期设置为事件的结束时间。对于全天事件，截止日期将设置为事件日期。对于有时间的事件，截止日期将包含结束时间。",
				},
				recurringEventRelatedNotesMode: {
					name: "重复事件的关联笔记",
					description:
						"选择关联到外部日历事件某一次重复的笔记，是显示在已加载的整个系列中，还是仅显示在所选实例上。",
					options: {
						series: "整个系列",
						instance: "仅所选实例",
					},
			...
			autoExport: {
				...
				excludeCompleted: {
					name: "排除已完成任务",
					description:
						"启用后，已完成任务会从 ICS 导出中省略。已完成状态来自任务状态设置。",
				},
				excludeArchived: {
					name: "排除已归档任务",
			...
			googleCalendarExport: {
				...
				defaultReminder: {
					name: "默认提醒",
					description:
						"为带时间的 Google Calendar 事件添加弹出提醒。请输入事件开始前的分钟数，用逗号分隔。留空则使用日历默认值。常用值：15、30、60、1440。",
				},
			...
			httpApi: {
				...
				mcp: {
					enable: {
						name: "启用 MCP 服务器",
						description:
							"通过 /mcp 端点使用 Model Context Protocol 暴露 TaskNotes 工具。需要启用 HTTP API。",
					},
				},
				endpoints: {
					header: "可用API端点",
			...
			mdbaseSpec: {
				...
				enable: {
					name: "生成 mdbase 类型定义",
					description:
						"随着设置变化，在库根目录生成并维护 mdbase 类型文件（mdbase.yaml 和 _types/task.md）。",
				},
			},
			timeFormats: {
				justNow: "刚刚",
	...
	goals: {
		view: {
			title: "目标",
			goalCountOne: "{count} 个",
			goalCountOther: "{count} 个",
			filters: { all: "全部", active: "进行中", paused: "已暂停", done: "已达成" },
			periods: { day: "日", week: "周", month: "月", year: "年" },
			previousPeriod: "上一个周期",
			nextPeriod: "下一个周期",
			backToCurrent: "回到当前",
			historyReadonly: "历史视图 · 只读模式",
			columns: {
				goal: "目标",
				type: "类型",
				today: "今日",
			...
			childCountOne: "{count} 子",
			childCountOther: "{count} 子",
			ownedByChildren: "由子目标持有",
			openGoal: "查看目标 {name}",
			countActual: "{actual} 次",
			countProgress: "{actual} / {target} 次",
			milestones: "里程碑",
			milestoneSubtitle: "累计数据 · 不随上方时间范围变化，包含之前各周",
			milestoneSummary: "{count} 条 · 已达成 {complete}",
			milestoneSummaryStale: "{count} 条 · 已达成 {complete} · {stale} 条待更新",
			noMilestones: "暂无里程碑，可在目标详情中添加结果指标或完成项",
		},
		management: "目标管理",
		newGoal: "新增目标",
		refresh: "刷新",
		paused: "已暂停",
		emptyTitle: "给这些任务定个去处",
		emptyDescription: "已经记录的时间，正等着被放进一个目标里。",
		createGoal: "创建目标",
		cancel: "取消",
		name: "名称",
		type: "类型",
		...
		target: "目标值",
		milestone: "里程碑（可选）",
		milestoneName: "结果指标名称",
		tiers: "档位，例如 100, 1000, 5000",
		files: "个目标文件",
		created: "目标已创建",
		createFailed: "目标创建失败",
		noGoals: "还没有目标",
		...
		aggregate: "合计",
		noMilestone: "无里程碑",
		confirmAchievement: "确认达成",
		confirmAchievementDescription: "将记录今天为达成日，并写入累计投入时长。",
		continue: "继续",
		undo: "撤销达成",
		openFile: "打开目标笔记",
		notDesktop: "目标管理仅在桌面端提供。",
	},
	notices: {
		languageChanged: "语言已更改为{language}。",
		releaseAvailable: {
			message: "TaskNotes Time {version} 可用。",
			action: "在社区插件中打开",
		},
	...
	commands: {
		...
		openGoalsView: "打开目标管理",
		openGoalBoard: "打开目标看板",
		createNewTask: "创建新任务",
	...
	modals: {
		...
		migration: {
			title: "迁移到新的循环系统",
			description:
				"TaskNotes 现在使用行业标准的 RRULE 模式进行循环，可以创建更复杂的计划并与其他应用更好地兼容。",
